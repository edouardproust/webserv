...
CgiHandler::CgiHandler(Request const& request, CgiParams const& cgiParams)
: _request(request)
...
pid_t	CgiHandler::_forkAndExec() const
{
	...
	if (pid == 0) { // Child
		_redirectIoInChild();
		execve(_cgiParams.getExecutor().c_str(), _cgiParams.getArgv().data(), _cgiParams.getEnvp().data());
...
void	CgiHandler::_prepareIo()
{
	...
	std::string const& body = _request.getBody();
	if (method == "POST" || method == "PUT") {
		ssize_t n = write(_stdinPipe[1], body.c_str(), body.size());
...
CgiParams const&	CgiHandler::getCgiParams() const
{
	return _cgiParams;
}
