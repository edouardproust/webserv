...
void Network::_onCatchRequest(HostPortPair const& srcPort, std::string const& rawRequest) const {
	Request request(rawRequest);
	std::string const& method = !request.getMethod().empty() ? (request.getMethod() + " ") : "";
	std::cerr << "[INFO] Network: caught " << method << "request on port " << srcPort << std::endl;
	if (DEVMODE) std::cerr << request << std::endl;
	Response const response = Router::dispatchRequest(_config, request, srcPort);
	if (DEVMODE) std::cerr << response << std::endl;
	_sendResponse(srcPort, response);
}
...
void	Network::_runManualTests() const {
	...
	_onCatchRequest(srcPort, "GET /cgi-bin/test.py HTTP/1.1\r\nHost: localhost:8080\r\n\r\n"); // valid python script
