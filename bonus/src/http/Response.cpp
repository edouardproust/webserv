...
std::ostream& operator<<(std::ostream& os, Response const& response)
{
	...
	os << "- Set-Cookie Headers: " << response.getSetCookies().size() << "\n";
	const std::vector<std::string>& setCookies = response.getSetCookies();
	for (std::vector<std::string>::const_iterator it = setCookies.begin();
		it != setCookies.end(); ++it)
			os << "  - Set-Cookie: " << PrintableString(*it) << "\n";
